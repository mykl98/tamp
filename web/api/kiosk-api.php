...
if($_POST){
    ...
    function getSchoolDetail(){
        ...
        $table = "school";
        $data = array();
        $sql = "SELECT idx,id,image,name,address,color FROM `$table`";
        if($result=mysqli_query($conn, $sql)){
            if(mysqli_num_rows($result) > 0){
                ...
                $value -> positions = getPositionList();
                $value -> grades = getGradeList();
                $value -> sections = getSectionList();
                array_push($data, $value);
                $data = json_encode($data);
                return "true*_*" . $data;
            }else{
    ...
    function getCardDetail($cardId){
        ...
        $data = array();
        $table = "account";
        $sql = "SELECT idx,image,firstname,lastname,saccess FROM `$table` WHERE id='$cardId' && saccess!='accesscard' && status='active'";
        if($result=mysqli_query($conn, $sql)){
            if(mysqli_num_rows($result) > 0){
                $row = mysqli_fetch_array($result);
                $value = new \StdClass();
                ...
                $table = "student";
                $sql = "SELECT idx,image,firstname,lastname,grade,section,test FROM `$table` WHERE id='$cardId' && status='active'";
                if($result=mysqli_query($conn,$sql)){
                    if(mysqli_num_rows($result) > 0){
                        $row = mysqli_fetch_array($result);
                        $studentIdx = $row["idx"];
    ...
    function getAnnouncementList($schoolIdx){
        global $conn;
        $data = array();
        $table = "announcement";
        $sql = "SELECT announcement FROM `$table` WHERE school='$schoolIdx' && status='active'";
        if($result=mysqli_query($conn, $sql)){
            if(mysqli_num_rows($result) > 0){
                while($row=mysqli_fetch_array($result)){
                    $value = new \StdClass();
    ...
    switch($activity){
        case "get-school-detail":
            echo getSchoolDetail();
            break;
        case "get-card-detail":
            $cardId = sanitize($_POST["cardid"]);
            $schoolIdx = sanitize($_POST["schoolidx"]);
            echo getCardDetail($cardId);
            break;
        case "get-announcement":
            $check = checkSchoolStatus();
            if($check == "true"){
                echo "locked";
                return;
            }
            $schoolIdx = sanitize($_POST["schoolidx"]);
            echo getAnnouncementList($schoolIdx);
            break;
        default: 
            echo "System Error!";
            break;
